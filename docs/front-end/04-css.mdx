...
<Tabs groupId="icl-versions">
  <TabItem value="v3.0" label="ICL Version 3+ (Astro)">
    The main screen css file is linked to in `main.ts` at `src/assets/js/main.ts` using an import statement. This allows it to be processed correctly by Astro during development and build (it is not included in the main built js file). It is included as the last import statement: 
    ```js
    // Import the main CSS file for the application (keep this line here after other imports)
    import "../css/index.css";
    ```
    The main print css file is linked to in `Layout.astro` at `src/layouts/Layout.astro`: 
    ```html
    <link rel="stylesheet" href="/print.css" media="print" />
    ```
  </TabItem>
  <TabItem value="v2.0" label="ICL Version 2+ (Vite/PostCSS)">
    The main screen and print css files are linked to in `_head.twig` at `source/_meta/_head.twig`: 
    ```html
    <link rel="stylesheet" href="../../dist/assets/style.css" media="screen">
    <link rel="stylesheet" href="../../css/print.css?{{ cacheBuster }}" media="print">
    ```
  </TabItem>
  <TabItem value="v1.0" label="ICL Version < 2 (SCSS)">
    The main screen and print css files are linked to in `_head.twig` at `source/_meta/_head.twig`: 
    ```html
    <link rel="stylesheet" href="../../css/style.css?{{ cacheBuster }}" media="screen">
    <link rel="stylesheet" href="../../css/print.css?{{ cacheBuster }}" media="print">
    ```
  </TabItem>
</Tabs>
...
<Tabs groupId="icl-versions">
  <TabItem value="v2.0" label="ICL Version 2+ (Astro & Pattern Lab Vite/PostCSS Version)">
  ...
  <TabItem value="v1.0" label="ICL Version < 2 (SCSS)">
    ...
    - Partials within the **abstracts directory** are using `@forward` to ensure all of the (typically not output) rules are available across all stylesheets:
...
<Tabs groupId="icl-versions">
  <TabItem value="v3.0" label="ICL Version 3+ (Astro)">
    A print-specific stylesheet is in use at `src/assets/css/print.css` to disable page elements irrelevant for printing (site header, site footer, subnav, etc) and to output the url of links within page body text.
  </TabItem>
  <TabItem value="v2.0" label="ICL Version 2+ (Vite/PostCSS)">
