...
<Tabs groupId="icl-versions">
  <TabItem value="v3.0" label="ICL Version 3+ (Astro)">
    ...
    ![XD SVG](_media/icl-interface.jpg)
    ...
    To add a component or page to the list, see [Adding A New Page to the ICL List](#adding-a-new-page-to-the-icl-list)
  ...
  <TabItem value="v2.0" label="ICL Version < 3 (Pattern Lab)">
    The Pattern Lab and Fractal interfaces are created from these respective platforms and cannot be edited.
...
```
Where "compoennt name" is the desired name of the component being created.
This command will do perform a number of tasks:
...
**Running the [command above](#generating-components-in-astro) will create this page. However, there may be some times when a page is needed to be created without running the above command.**
...
```jsx title="src/components/_icl-component-page-list/ICL-Component-Page-List.astro" showLineNumbers
...
```jsx title="src/pages/[kitchenSink].astro" showLineNumbers
...
- The second item `kitchenSink: 'kitchen-sink-with-subnav'` shows a path of `/kitchen-sink-with-subnav`
— The third item `kitchenSink: 'kitchen-sink-hero-no-image'` shows a path of `/kitchen-sink-hero-no-image`
- And so on...

These urls can be added to `ICL-Component-Page-List.astro` [similarly to the above example](#page-without-variations), but in a nested `<details>` element:
```jsx title="src/pages/[kitchenSink].astro" showLineNumbers
<li>
...
<li>
  <details class="icl-component-page-list__variations-details">
    ...
    <ul>
      ...
      // rest of code
```
- `line 2`: Add a link to the "default" (or "non-variant") version of the page
— `line 5` & Onward: Add a new `<details>` element with links in a nested `<ul>` with the variant links